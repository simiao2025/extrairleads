...
export default function LoginPage() {
	...
	useEffect(() => {
		...
		if (successParam === "email_verified") {
			setSuccess(
				"Sua conta foi ativada com sucesso! Faça login abaixo para começar.",
			);
		}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (showForgot) {
			...
			try {
				...
				if (!result.success) {
					setForgotError(
						result.error || "Erro ao enviar e-mail de recuperação.",
					);
					return;
				}
			...
			<div className="hidden lg:flex flex-col justify-between p-12 relative overflow-hidden bg-[#050505] border-r border-white/5">
				...
				<div className="relative z-10 space-y-6 max-w-xl animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-300 fill-mode-both">
					...
					<p className="text-zinc-400 text-lg leading-relaxed max-w-md">
						Identifique, qualifique e converta contas enterprise de forma
						invisível. Nossa inteligência artificial trabalha em milissegundos
						para lotar seu pipeline.
					</p>
				</div>
