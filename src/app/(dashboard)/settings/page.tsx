...
export default function SettingsPage() {
  const [whatsappStatus, setWhatsappStatus] = useState<{ connected: boolean; state: string; instanceName?: string } | null>(null);
  ...
  const integrations = [
    { name: "Banco de Dados Neon Postgres", key: envStatus.database, icon: Server, desc: "Armazenamento na nuvem multi-tenant seguro" },
    { name: "Google Maps (SerpApi)", key: envStatus.serpapi, icon: Lock, desc: "Motor de busca de novos leads comerciais" },
    { name: "Llama 3.3 (Groq API)", key: envStatus.groq, icon: Cpu, desc: "Análise neural e geração de abordagens de vendas" },
    { name: "Evolution Go v3 (WhatsApp)", key: envStatus.evolution, icon: Smartphone, desc: "Motor de disparos ultra-veloz em Go" },
  ];
  ...
  const handleGenerateQr = async () => {
    ...
    try {
      ...
      if (res.success && res.qrCode) {
        ...
        setQrError(res.error || "Não foi possível carregar o QR Code. Certifique-se de que a instância está criada.");
      }
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto space-y-8 relative z-10 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-900 pb-6">
          <div>
            ...
            <p className="text-zinc-400 text-sm mt-1">Gerencie a conexão de WhatsApp da sua conta e confira o status das integrações.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-[#0c0c0e] border border-zinc-800 rounded-2xl p-6 shadow-xl">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-zinc-500 text-xs mt-0.5">Emparelhe seu celular para realizar os envios dinâmicos das campanhas.</p>
                  </div>
                </div>
                ...
                >
                  <RefreshCw className={`w-4 h-4 ${checking ? "animate-spin text-emerald-500" : ""}`} />
                </button>
              </div>
              ...
              <div className="bg-zinc-950 border border-zinc-900 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                  <span className="text-zinc-500 text-[11px] font-semibold uppercase tracking-wider">Status Atual</span>
                  <div className="flex items-center gap-2.5 mt-1">
                    {whatsappStatus?.connected ? (
                      <>
                        <div className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
                        <span className="text-emerald-400 font-bold text-sm bg-emerald-400/5 border border-emerald-400/10 px-3 py-0.5 rounded-full">🟢 CONECTADO</span>
                      </>
                    ) : (
                      <>
                        <div className="w-2.5 h-2.5 rounded-full bg-red-400" />
                        <span className="text-red-400 font-bold text-sm bg-red-400/5 border border-red-500/10 px-3 py-0.5 rounded-full">🔴 DESCONECTADO</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                {whatsappStatus?.instanceName && (
                  <div>
                    <span className="text-zinc-500 text-[11px] font-semibold uppercase tracking-wider">Identificador da Instância</span>
                    <p className="text-zinc-300 font-mono text-sm mt-1">{whatsappStatus.instanceName}</p>
                  </div>
                )}
              </div>
                ...
                {whatsappStatus?.connected ? (
                  <div className="text-center py-6 space-y-4">
                    ...
                    <div className="max-w-sm mx-auto space-y-2">
                      ...
                      <p className="text-zinc-400 text-sm leading-relaxed">
                        Sua conexão individual no Evolution Go v3 está operando perfeitamente. Todas as suas campanhas e fluxos de prospecção neural serão disparados por este canal de forma segura.
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                    <div className="space-y-4">
                      <h3 className="font-bold text-white text-base">Como conectar seu WhatsApp?</h3>
                      <ol className="text-zinc-400 text-xs space-y-2.5 list-decimal pl-4 leading-relaxed">
                        <li>Clique no botão <strong className="text-zinc-200">"Gerar QR Code"</strong> ao lado.</li>
                        <li>Abra o WhatsApp em seu aparelho celular.</li>
                        <li>Toque em <strong className="text-zinc-200">Aparelhos Conectados</strong> &gt; <strong className="text-zinc-200">Conectar Aparelho</strong>.</li>
                        <li>Aponte a câmera do seu celular para a tela para escanear o QR Code gerado.</li>
                        <li>O sistema identificará e ativará o canal automaticamente em até 5 segundos após a leitura.</li>
                      </ol>
                      ...
                      {qrError && (
                        <p className="text-red-400 text-xs mt-2 bg-red-400/5 border border-red-500/10 p-2 rounded-lg">{qrError}</p>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center p-4 border border-zinc-800 bg-[#0c0c0e] rounded-xl min-h-[220px] relative">
                      {qrLoading ? (
                        ...
                        <div className="text-center space-y-2 text-zinc-600 max-w-[180px]">
                          ...
                          <p className="text-[11px] leading-relaxed">Clique em gerar para exibir o código QR de sincronização.</p>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            <div className="bg-[#0c0c0e] border border-zinc-800 rounded-2xl p-6 shadow-xl">
              ...
              <p className="text-zinc-500 text-xs mb-6">Status dos motores e credenciais de inteligência artificial de faturamento global do app.</p>

              <div className="space-y-4">
                {integrations.map((integration, i) => (
                  <div key={i} className="flex flex-col p-3.5 rounded-xl bg-zinc-950 border border-zinc-900 gap-3">
                    <div className="flex items-center gap-3">
                      <div className="p-1.5 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-400">
                        <integration.icon className="w-4 h-4" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-xs font-semibold text-white truncate">{integration.name}</p>
                        <p className="text-[10px] text-zinc-500 truncate mt-0.5">{integration.desc}</p>
                      </div>
                    </div>

                    <div className="flex items-center justify-between border-t border-zinc-900/50 pt-2.5">
                      <span className="text-[10px] text-zinc-600 font-semibold uppercase tracking-wider">Status</span>
        ...
        <div className="text-center pt-8 border-t border-zinc-900">
          <p className="text-xs text-zinc-600 leading-relaxed">
            Todas as chaves de acesso são mantidas em ambiente de isolamento criptográfico seguro.<br />
