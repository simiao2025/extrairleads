...
export default function SettingsPage() {
	...
	const [provider, setProvider] = useState<"evolution" | "meta_official">(
		"evolution",
	);
		...
		const loadSettings = async () => {
			...
			if (res.success) {
				...
				localStorage.setItem(
					"soundEnabled",
					res.notificationsEnabled !== false ? "true" : "false",
				);
				setProfileInfo({
	...
	const handleChangePassword = async (e: React.FormEvent) => {
		...
		if (!currentPassword || !newPassword || !confirmPassword) {
			notify("Preencha todos os campos para alterar a senha.", {
				type: "error",
			});
			return;
		}
		if (newPassword.length < 6) {
			notify("A nova senha deve ter pelo menos 6 caracteres.", {
				type: "error",
			});
			return;
		}
			...
			<div className="max-w-5xl mx-auto space-y-8 relative z-10 animate-in fade-in duration-500">
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-900 pb-6">
					<div>
						...
						<p className="text-zinc-400 text-sm mt-1">
							Gerencie sua assinatura, conexão de WhatsApp e status das
							integrações.
						</p>
					</div>
				</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
						...
						<div className="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl p-6 shadow-xl">
								...
								<div className="flex items-center gap-3">
									...
									<div>
										<h2 className="text-lg font-bold text-white font-heading">
											Canal de Disparo WhatsApp
										</h2>
										<p className="text-zinc-500 text-xs mt-0.5">
											Emparelhe seu celular para realizar os envios dinâmicos
											das campanhas.
										</p>
									</div>
								</div>
							...
							<div className="mb-6 flex flex-col md:flex-row items-start md:items-center gap-4 p-4 bg-zinc-950 border border-zinc-900 rounded-xl">
								<div className="flex-1">
									...
									<p className="text-xs text-zinc-500">
										Escolha por onde o sistema enviará as mensagens.
									</p>
								</div>
							...
							{provider === "evolution" ? (
											...
											<div className="text-center py-6 space-y-4">
												...
												<div className="max-w-sm mx-auto space-y-2">
													...
													<p className="text-zinc-400 text-sm leading-relaxed">
														Sua conexão individual no Evolution Go v3 está
														operando perfeitamente. Todas as suas campanhas e
											...
											<div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
												<div className="space-y-4">
													...
													<ol className="text-zinc-400 text-xs space-y-2.5 list-decimal pl-4 leading-relaxed">
														...
														<li>
															Aponte a câmera do seu celular para a tela para
															escanear o QR Code gerado.
														</li>
														<li>
															O sistema identificará e ativará o canal
															automaticamente em até 5 segundos após a leitura.
														</li>
													</ol>
													...
													{qrLoading ? (
														...
														<div className="text-center space-y-2 text-zinc-600 max-w-[180px]">
															...
															<p className="text-[11px] leading-relaxed">
																Clique em gerar para exibir o código QR de
																sincronização.
															</p>
														</div>
													)}
								...
								<div className="border border-blue-900/30 bg-blue-950/10 rounded-xl p-6 space-y-5">
									<div>
										<h3 className="font-bold text-white text-base text-blue-400">
											Credenciais Meta Cloud API
										</h3>
										<p className="text-xs text-zinc-400 mt-1">
											Preencha com os dados do painel do Facebook Developers.
										</p>
									</div>
							...
							<div className="flex items-center justify-between p-4 bg-zinc-900 rounded-xl border border-zinc-800">
								<div>
									...
									<p className="text-xs text-zinc-400">
										Tocar som ao receber novas notificações ou mensagens (via
										Toast).
									</p>
								</div>
					...
					<div className="space-y-6">
							...
							<div className="relative z-10">
								...
								<div className="space-y-4">
									<div>
										<div className="flex justify-between text-sm mb-1.5">
											<span className="text-zinc-400 font-medium">
												Saldo de Leads (Créditos)
											</span>
											<span className="text-emerald-400 font-mono font-bold">
												{profileInfo.leadsBalance} Restantes
											</span>
										</div>
										<div className="w-full h-2 bg-zinc-800 rounded-full overflow-hidden shadow-inner">
											<div
												className="h-full bg-emerald-500 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.8)] transition-all duration-1000"
												style={{
													width: `${Math.min(100, (profileInfo.leadsBalance / 500) * 100)}%`,
												}}
											/>
										</div>
										<p className="text-[11px] text-zinc-500 mt-2">
											Os créditos são consumidos a cada busca de leads realizada
											pelo robô.
										</p>
									</div>
									...
									<div className="pt-4 mt-4 border-t border-zinc-800/80">
										...
										<button
											onClick={() =>
												window.open("https://pay.kiwify.com.br/login", "_blank")
											}
						...
						{/* Integrations Card */}
						<div className="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl p-6 shadow-xl">
							<h2 className="text-lg font-bold text-white mb-1.5 font-heading">
								Integrações Globais
							</h2>
							<p className="text-zinc-500 text-xs mb-6">
								Status dos motores e credenciais de inteligência artificial de
								faturamento global do app.
							</p>
				...
				<div className="text-center pt-8 border-t border-zinc-900">
					<p className="text-xs text-zinc-600 leading-relaxed">
						Todas as chaves de acesso são mantidas em ambiente de isolamento
						criptográfico seguro.
						<br />
						Para modificações de arquitetura do sistema, entre em contato com o
						suporte técnico.
					</p>
				</div>
			</div>
					...
					<div className="bg-zinc-950 border border-zinc-800 rounded-3xl p-8 max-w-lg w-full relative z-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
						<div className="text-center mb-8">
							...
							<h2 className="text-2xl font-bold text-white font-heading mb-2">
								Desbloqueie Mais Leads
							</h2>
							<p className="text-zinc-400 text-sm">
								Adicione créditos avulsos que{" "}
								<strong className="text-zinc-200">não expiram</strong> e
								mantenha seu Motor de Vendas acelerando.
							</p>
						</div>
						...
						<div className="space-y-3 mb-8">
							{[
								...
								>
									<div className="flex items-center gap-3">
										<div
											className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${selectedPackIndex === idx ? "border-emerald-500" : "border-zinc-700 group-hover:border-emerald-500"}`}
										>
											<div
												className={`w-2.5 h-2.5 rounded-full bg-emerald-500 transition-opacity ${selectedPackIndex === idx ? "opacity-100" : "opacity-0 group-hover:opacity-50"}`}
											/>
										</div>
							...
							onClick={() => {
								...
								setTimeout(() => {
									...
									if (selectedLink.startsWith("#")) {
										notify(
											"Aviso: O link de checkout da Kiwify ainda não foi configurado.",
											{
												type: "info",
											},
										);
										...
										window.open(
											`${selectedLink}?email=${encodeURIComponent(profileInfo.email)}`,
											"_blank",
										);
									}
