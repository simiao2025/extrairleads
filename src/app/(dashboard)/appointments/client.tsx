...
function AppointmentDialog({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (result.success) {
			success(
				appointment?.id ? "Agendamento atualizado!" : "Agendamento criado!",
			);
			onOpenChange(false);
			onSuccess();
		} else {
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="bg-zinc-950/90 backdrop-blur-2xl border-zinc-800 text-white max-w-sm">
				<DialogHeader>
					<DialogTitle className="text-xl font-black text-red-400">
						Confirmar Exclusão
					</DialogTitle>
				</DialogHeader>
			...
			<>
				<div className="text-center py-16">
					...
					<p className="text-sm text-zinc-500 mb-6">
						Crie seu primeiro agendamento para acompanhar seus leads
						interessados.
					</p>
					<Button
		...
		<>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{appointments.map((apt) => {
					const status =
						statusConfig[apt.status || "confirmed"] || statusConfig.confirmed;
