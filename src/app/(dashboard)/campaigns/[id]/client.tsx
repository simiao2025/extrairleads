...
export function CampaignDetailsClient({
	...
	const handleQualifyLeads = async () => {
		...
		try {
			...
			if (res.success) {
				success(
					`Sucesso! ${res.count || 0} leads estão sendo qualificados em segundo plano.`,
				);
				await refreshData();
			} else {
			...
			<div className="max-w-7xl mx-auto space-y-8">
				...
				<div className="bg-zinc-900/20 border border-zinc-800/40 p-6 rounded-2xl space-y-4">
					<h3 className="text-xs font-black uppercase tracking-widest text-zinc-400 flex items-center gap-1.5">
						<Zap className="w-4 h-4 text-emerald-400" /> Ações em Lote
						Disponíveis
					</h3>
					<p className="text-xs text-zinc-500">
						Dispare fluxos automatizados em massa dedicados apenas para os
						contatos desta campanha.
					</p>
