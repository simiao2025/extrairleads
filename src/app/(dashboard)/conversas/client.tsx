...
export function ConversasClient({ initialConversations }: ConversasClientProps) {
  ...
  const activeConversation = conversations.find((c) => c.lead.id === activeLeadId);
  ...
  // Carrega o chat de um lead específico
  const loadChat = async (leadId: number, showLoading = true) => {
    if (showLoading) setLoadingChat(true);
    const data = await getLeadChatAction(leadId);
  ...
  // Rola para o fim ao carregar histórico
  useEffect(() => {
    scrollToBottom();
  }, [scrollToBottom]);

  // Polling para novas mensagens da conversa ativa
  useEffect(() => {
    let interval: any;
    if (activeLeadId) {
      loadChat(activeLeadId, true);
      interval = setInterval(() => {
        loadChat(activeLeadId, false);
      }, 4000);
    }
    return () => clearInterval(interval);
  }, [activeLeadId, loadChat]);

  // Polling para atualizar a lista de conversas inteira em segundo plano
  useEffect(() => {
    const interval = setInterval(async () => {
      // Importa a action do app para obter a lista atualizada de conversas
  ...
  const handleToggleIntervention = async () => {
    ...
    const isCurrentlyIntervention = activeConversation.lead.status === "human_intervention";
    const nextStatus = isCurrentlyIntervention ? "contacted" : "human_intervention";
    ...
    if (res.success) {
      ...
      setConversations((prev) =>
        prev.map((c) =>
          c.lead.id === activeLeadId ? { ...c, lead: { ...c.lead, status: nextStatus } } : c,
        ),
      );
      success(
          ...
          {filteredConversations.length === 0 ? (
            <div className="text-center py-16 text-zinc-500 px-4">
              ...
              <p className="text-xs mt-1">
                Os leads aparecerão aqui assim que trocarem mensagens no WhatsApp.
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      <p className="text-xs text-zinc-400 truncate w-3/4">
                        {c.lastMessage.role === "assistant" ? (
                          <span className="text-emerald-500 font-medium">Você: </span>
                        ) : null}
        ...
        {activeConversation ? (
            ...
            <div className="bg-[#202c33] px-4 py-3 flex items-center justify-between border-b border-zinc-800/40 z-10 shrink-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button
                  ...
                  title="Dados do Lead"
                >
                  <Zap className={`w-4 h-4 ${showRightPanel ? "text-emerald-400" : ""}`} />
                </Button>
              </div>
            </div>
            ...
            {activeConversation.lead.status === "human_intervention" && (
              <div className="bg-amber-500/10 border-b border-amber-500/20 px-4 py-2 flex items-center justify-between text-xs text-amber-300 z-10 shrink-0 backdrop-blur-md">
                <span className="flex items-center gap-1.5 font-medium">
                  <HandMetal className="w-3.5 h-3.5 text-amber-400 animate-bounce" /> O robô IA SDR
                  está temporariamente desligado para este lead. Suas respostas serão enviadas
                  manualmente.
                </span>
                <button
                  onClick={handleToggleIntervention}
              ...
              {loadingChat ? (
                <div className="flex items-center justify-center h-full">
                  <div className="bg-[#202c33] px-4 py-2 rounded-full text-xs text-zinc-400 flex items-center gap-2 shadow-lg">
                    <Loader2 className="w-3.5 h-3.5 animate-spin text-emerald-400" /> Sincronizando
                    chat...
                  </div>
                </div>
              ) : history.length === 0 ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 z-10">
            ...
            <p className="text-sm text-zinc-500 max-w-sm">
              Selecione uma conversa na barra lateral esquerda para acompanhar e intervir em tempo
              real.
            </p>
          </div>
        )}
          ...
          <div className="space-y-4 pb-6">
            ...
            <div className="bg-[#202c33]/50 p-4 rounded-2xl border border-zinc-800/60 shadow-inner">
                ...
                activeConversation.lead.aiScore !== undefined ? (
                  <span
                    className={`font-black text-xs px-2 py-0.5 rounded-lg ${
                      activeConversation.lead.aiScore >= 80 || activeConversation.lead.aiScore >= 8
              ...
              <p className="text-xs text-zinc-300 leading-relaxed italic border-l-2 border-emerald-500/60 pl-3">
                &ldquo;
                {activeConversation.lead.aiAnalysis || "Sem análise técnica disponível no momento."}
                &rdquo;
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 pt-6 pb-6">
            ...
            <div className="space-y-3.5 text-xs">
              <div className="flex items-start gap-2.5 text-zinc-300">
                ...
                <div>
                  <p className="text-zinc-500 text-[10px] uppercase font-bold">Localização</p>
                  <p className="text-zinc-200 mt-0.5">
                    {activeConversation.lead.city || "Não informado"}
                    {activeConversation.lead.city && activeConversation.lead.state ? ", " : ""}
              ...
              <div className="flex items-start gap-2.5 text-zinc-300">
                ...
                <div>
                  <p className="text-zinc-500 text-[10px] uppercase font-bold">Nicho / Segmento</p>
                  <Badge
                    variant="outline"
              ...
              {activeConversation.lead.website && (
                <div className="flex items-start gap-2.5 text-zinc-300">
                  ...
                  <div className="min-w-0">
                    <p className="text-zinc-500 text-[10px] uppercase font-bold">Site</p>
                    <a
                      href={
              ...
              <div className="flex items-start gap-2.5 text-zinc-300">
                ...
                <div>
                  <p className="text-zinc-500 text-[10px] uppercase font-bold">Adicionado em</p>
                  <p className="text-zinc-200 mt-0.5">
                    {activeConversation.lead.createdAt
                      ? new Date(activeConversation.lead.createdAt).toLocaleDateString([], {
                          day: "2-digit",
                          month: "long",
                          year: "numeric",
                        })
