...
export default function KnowledgeBaseTab() {
	...
	const handleDelete = async (id: number) => {
		if (
			!confirm(
				"Tem certeza que deseja remover este documento da base de conhecimento do Agente?",
			)
		)
			return;
		setError("");
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-sm text-zinc-500 font-medium">
								Cadastre FAQs, serviços e regras para o seu Agente IA consultar
								em tempo real.
							</p>
						</div>
					</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
						...
						<Card className="border-white/[0.06] bg-white/[0.02] backdrop-blur-xl relative overflow-hidden group">
							...
							<CardHeader>
								<CardTitle className="text-lg font-bold flex items-center gap-2">
									<FileUp className="w-5 h-5 text-emerald-400" /> Upload de
									Documento
								</CardTitle>
								<CardDescription className="text-xs">
									Faça upload de um arquivo PDF, TXT, MD ou Planilha Excel
									(.xls, .xlsx). O sistema extrairá o texto e adicionará à base
									vetorial.
								</CardDescription>
							</CardHeader>
					...
					<FadeIn delay={0.2}>
						<Card className="border-white/[0.06] bg-white/[0.02] backdrop-blur-xl relative overflow-hidden group">
							...
							<CardHeader>
								...
								<CardDescription className="text-xs">
									As informações inseridas abaixo serão convertidas em vetores
									matemáticos e injetadas na inteligência artificial do SDR.
								</CardDescription>
							</CardHeader>
							<CardContent className="relative space-y-4">
				...
				<div className="lg:col-span-7 space-y-6">
					<h2 className="text-lg font-bold text-zinc-300 flex items-center gap-2">
						<FileText className="w-5 h-5 text-emerald-400" /> Base Cadastrada (
						{documents.length})
					</h2>
					...
					{loading ? (
						<div className="flex flex-col items-center justify-center py-20 gap-3">
							...
							<p className="text-zinc-500 text-sm">
								Vasculhando base vetorial no Postgres...
							</p>
						</div>
					) : documents.length === 0 ? (
						<FadeIn>
							<div className="text-center py-16 px-4 rounded-2xl border border-white/[0.04] bg-white/[0.01] backdrop-blur-sm">
								...
								<p className="text-zinc-600 text-xs mt-1 max-w-sm mx-auto">
									Seu SDR usará apenas o prompt básico. Preencha o formulário ao
									lado para dar respostas inteligentes de produtos e FAQ à IA.
								</p>
							</div>
						</FadeIn>
					) : (
