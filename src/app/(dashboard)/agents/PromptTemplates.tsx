    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4">
        {TEMPLATES.map((template, i) => (
          <Card key={i} className="bg-zinc-900/40 border-zinc-800 hover:border-emerald-500/30 transition-all group">
            <CardHeader className="pb-2">
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-sm font-bold text-emerald-400">{template.name}</CardTitle>
                  <CardDescription className="text-xs text-zinc-500">{template.description}</CardDescription>
                </div>
                <Button 
                  variant="ghost" 
                  size="icon" 
