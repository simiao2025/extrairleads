...
export async function GET(req: Request) {
	...
	const VERIFY_TOKEN =
		process.env.META_WEBHOOK_VERIFY_TOKEN || "extrairleads2026";
				...
				if (change.field === "messages" && change.value.messages) {
					...
					for (const message of change.value.messages) {
						...
						const formattedHistory = history
							.reverse()
							.map(
								(h) => `${h.role === "user" ? "Cliente" : "SDR"}: ${h.content}`,
							)
							.join("\n");
						...
						const tools = [
							{
								...
								function: {
									...
									description:
										"Atualiza os dados e muda status para qualificado ('qualified').",
									parameters: {
										type: "object",
										properties: {
											name: { type: "string" },
							...
							for (const toolCall of toolCalls) {
								...
								if (toolName === "update_lead_info") {
									...
									await db
										...
										.where(eq(leads.id, lead.id)); // Movimenta Kanban
