...
Fim do documento.
	...
	describe("parseExcelToMarkdown", () => {
		...
		it("should split large Excel sheets and replicate headers in each chunk", () => {
			...
			expect(chunks[0].title).toBe(
				"estoque.xlsx > Aba: Estoque (Parte 1 de 3)",
			);
			expect(chunks[1].title).toBe(
				"estoque.xlsx > Aba: Estoque (Parte 2 de 3)",
			);
			expect(chunks[2].title).toBe(
				"estoque.xlsx > Aba: Estoque (Parte 3 de 3)",
			);
	...
	describe("parseDocument router", () => {
		...
		it("should throw error for unsupported extension", async () => {
			...
			await expect(
				parseDocument(buffer, "imagem.png", "image/png"),
