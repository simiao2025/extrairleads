	...
	const handleClick = async () => {
		...
		try {
			...
			if (result.count === 0) {
				...
				error(result.error || "Erro ao executar a análise.");
			}
		} catch (_err) {
...
export function OutreachButton({
	...
	return (
		<Button
			...
			onClick={handleClick}
			disabled={
				!campaignId || isAutoOutreach || !isWhatsappConnected || loading
			}
		>
			<style>{`
        @keyframes shimmer {
      ...
      `}</style>
			...
			{!(!campaignId || isAutoOutreach || !isWhatsappConnected) && (
				<div className="absolute inset-0 z-0 pointer-events-none">
					<div
						className="w-12 h-full bg-gradient-to-r from-transparent via-emerald-300/30 to-transparent shadow-[0_0_15px_rgba(110,231,183,0.5)]"
...
export function FollowUpButton({
	...
	return (
		<Button
			...
			onClick={handleClick}
			disabled={
				!campaignId || !hasContactedLeads || !isWhatsappConnected || loading
			}
		>
			{loading ? (
				<Loader2 className="mr-2 h-4 w-4 animate-spin" />
