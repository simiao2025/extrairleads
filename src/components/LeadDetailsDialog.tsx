...
export default function LeadDetailsDialog({ lead, children }: { lead: Lead; children?: React.ReactNode }) {
      ...
      <DialogContent className="bg-[#0b141a] border-zinc-800 text-[#e9edef] max-w-5xl w-[95vw] h-[90vh] flex flex-col shadow-2xl p-0 overflow-hidden sm:rounded-2xl">
        ...
        <DialogHeader className="bg-[#202c33] px-4 py-3 flex flex-row items-center justify-between border-b border-zinc-800/50 shrink-0 m-0 space-y-0">
          <DialogTitle className="flex items-center gap-3 text-2xl w-full">
            ...
            <div className="flex flex-col items-start overflow-hidden w-full space-y-0.5">
              ...
              <p className="text-[11px] text-[#8696a0] font-normal leading-none">{lead.phone || "Sem telefone cadastrado"}</p>
            </div>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-1 overflow-hidden relative">
          ...
          <div className="hidden lg:flex w-1/3 flex-col bg-[#111b21] border-r border-zinc-800/50 z-10 p-5 overflow-y-auto">
            ...
            <h3 className="text-zinc-500 text-[10px] font-bold mb-3 uppercase tracking-widest">Dados do Lead</h3>
            <div className="space-y-3 text-xs">
              <div>
                <p className="text-zinc-500 text-[10px] uppercase">Localização</p>
                <p className="text-zinc-200 mt-0.5">{lead.city}, {lead.state}</p>
              </div>
              <div>
                <p className="text-zinc-500 text-[10px] uppercase">Nicho de Mercado</p>
                <p className="text-zinc-200 bg-zinc-800/50 inline-block px-2 py-1 rounded mt-1">{lead.niche || "Não informado"}</p>
              </div>
              {lead.website && (
                <div>
                  <p className="text-zinc-500 text-[10px] uppercase">Site</p>
                  <a href={lead.website.startsWith("http") ? lead.website : `https://${lead.website}`} target="_blank" rel="noreferrer" className="text-blue-400 hover:underline break-all mt-0.5 block">
                    {lead.website}
                  </a>
                </div>
              )}
          ...
          <div className="flex-1 flex flex-col z-10 relative bg-transparent">
            ...
            >
              ...
              <div className="lg:hidden bg-[#202c33]/90 backdrop-blur p-3 rounded-lg border border-zinc-800 text-[11px] text-zinc-300 mb-4 shadow-lg text-center mx-auto max-w-sm">
                <span className="font-bold text-[#00a884]">Análise IA ({lead.aiScore}): </span>
                {lead.aiAnalysis}
              </div>
              ...
              {loading ? (
                <div className="flex items-center justify-center h-full">
                  <div className="bg-[#202c33] px-4 py-2 rounded-full text-xs text-zinc-400 flex items-center gap-2 shadow-lg">
                    <Loader2 className="w-3 h-3 animate-spin" /> Sincronizando mensagens...
                  </div>
                </div>
              ) : history.length === 0 ? (
                <div className="flex items-center justify-center h-full">
            ...
            <div className="bg-[#202c33] px-4 py-3 flex items-end gap-2 border-t border-zinc-800/50">
              ...
              <div className="flex-1 bg-[#2a3942] rounded-xl overflow-hidden min-h-[40px] flex items-center border border-transparent focus-within:border-[#00a884]/50 transition-colors">
                <textarea
                  value={input}
                  ...
                  placeholder="Digite uma mensagem ou peça ajuda da IA..."
                  className="w-full bg-transparent text-[#e9edef] text-sm px-4 py-2.5 max-h-32 focus:outline-none resize-none min-h-[40px]"
