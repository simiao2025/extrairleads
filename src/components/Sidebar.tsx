...
import { checkWhatsAppConnectionAction, getWhatsAppSettingsAction } from "@/app/actions";
...
export default function Sidebar({ user }: SidebarProps) {
  ...
  const [liveBalance, setLiveBalance] = useState<number>(user?.leadsBalance ?? 0);
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0 flex items-center gap-2">
              <div className="w-8 h-8 flex items-center justify-center">
                <img src="/scraping.png" alt="ExtrairLeads" className="w-full h-full object-contain filter drop-shadow-[0_0_8px_rgba(16,185,129,0.3)]" />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3">
               ...
               {whatsappStatus && (
                  <div className={`w-2 h-2 rounded-full ${whatsappStatus.connected ? "bg-emerald-500 animate-pulse" : "bg-red-500"}`} />
               )}
               <button
        ...
        <div className="fixed inset-0 z-[60] bg-zinc-950 flex flex-col md:hidden pt-20 pb-[calc(env(safe-area-inset-bottom)+5rem)] px-6 overflow-y-auto">
          <div className="flex items-center gap-3 mb-8 bg-zinc-900/50 border border-zinc-800 rounded-2xl p-4">
             ...
             <div className="overflow-hidden">
                <p className="font-bold text-white text-base truncate">{user?.name || "Usuário"}</p>
      ...
      {/* DESKTOP SIDEBAR         */}
      {/* ======================= */}
      <aside className={`hidden md:flex md:flex-col shrink-0 border-r border-zinc-900 h-screen sticky top-0 z-50 transition-all duration-300 ${isCollapsed ? "w-[72px]" : "w-[240px]"} bg-zinc-950`}>
        
        <div className={`flex items-center relative py-5 border-b border-zinc-900/50 ${isCollapsed ? "justify-center px-0" : "px-6"}`}>
            ...
            return (
              <div key={link.name} className="flex flex-col gap-1">
                ...
                >
                  ...
                  <link.icon className={`w-[18px] h-[18px] shrink-0 ${isActive ? "text-emerald-500" : "text-zinc-500 group-hover:text-emerald-500/70"}`} />
        ...
        <div className={`flex flex-col gap-3 pb-6 border-t border-zinc-900/50 pt-5 ${isCollapsed ? "px-2" : "px-4"}`}>
          
          {/* Credits Box */}
          {isCollapsed ? (
            <div className="flex flex-col items-center justify-center gap-1.5 p-2 rounded-xl border border-zinc-800 bg-zinc-900/40 cursor-help" title={`${liveBalance} leads restantes`}>
              <span className="font-mono text-[10px] font-bold text-emerald-400">{liveBalance}</span>
              <Gem className="w-4 h-4 text-emerald-500" />
            </div>
          ) : (
            <div className="bg-zinc-900/40 border border-zinc-800 rounded-xl p-3.5">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Créditos</span>
                <span className="font-mono text-xs font-bold text-emerald-400">{liveBalance}</span>
              </div>
              <div className="w-full h-1.5 bg-zinc-800 rounded-full mb-2 overflow-hidden">
                <div className="h-full bg-emerald-500 rounded-full shadow-[0_0_8px_rgba(16,185,129,0.5)] transition-all" style={{ width: `${Math.min(100, (liveBalance / 500) * 100)}%` }} />
              </div>
              <p className="text-[10px] text-zinc-400 mb-3">{liveBalance} leads restantes</p>
              <Link 
                href="/settings" 
          ...
          {isCollapsed ? (
            <Link href="/settings" className="flex justify-center p-2 rounded-xl hover:bg-zinc-900/80 transition-colors border border-transparent hover:border-zinc-800" title={`Plano ${livePlan}`}>
              <Gem className="w-4 h-4 text-emerald-500" />
            </Link>
          ) : (
            <Link href="/settings" className="flex items-center justify-between rounded-xl hover:bg-zinc-900/80 transition-colors px-3 py-2 border border-transparent hover:border-zinc-800">
               <div className="flex items-center gap-2">
                 <Gem className="w-3.5 h-3.5 text-emerald-500" />
                 <span className="text-xs font-bold text-white">Plano {livePlan}</span>
               </div>
          ...
          <div className={`flex items-center mt-1 ${isCollapsed ? "justify-center flex-col gap-3 px-0 py-2" : "gap-3 px-3 py-2"}`}>
             ...
             {!isCollapsed && (
               <div className="flex-1 overflow-hidden">
                  <span className="block text-xs font-bold text-zinc-300 truncate">{user?.name || "Usuário"}</span>
                  <span className="block text-[10px] text-zinc-500 truncate">{user?.email}</span>
               </div>
             )}
             <button onClick={handleSignOut} className={`rounded-lg hover:bg-red-500/10 hover:text-red-400 text-zinc-500 transition-colors ${isCollapsed ? "p-2" : "p-1.5"}`} title="Sair">
