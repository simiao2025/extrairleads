...
export function ProgressModal({
	...
	const { data } = useSWR(
		jobId ? `/api/jobs/progress?jobId=${jobId}` : null,
		fetcher,
		{
	...
	const total = data?.totalExpected || 20;
			...
			<div className="bg-zinc-950 border border-zinc-800 p-8 rounded-2xl w-full max-w-md shadow-2xl relative overflow-hidden">
				...
				<div className="relative z-10 flex flex-col items-center text-center">
					...
					<p className="text-sm text-zinc-400 mb-8 max-w-[280px]">
						...
						{status === "completed" &&
							"Os leads validados já estão no seu pipeline."}
					</p>
					...
					<div className="w-full space-y-5">
						...
						<div className="space-y-2">
							<div className="flex justify-between text-xs font-bold uppercase tracking-wider">
								<span
									className={
										status === "scraping" ? "text-blue-400" : "text-zinc-500"
									}
								>
									1. Raspagem
								</span>
