...
export async function parseDocument(
  ...
  if (extension === "md" || mimeType === "text/markdown" || mimeType === "text/x-markdown") {
 ...
 */
export function parseMarkdownByHeaders(text: string, fileName: string): DocumentChunk[] {
  ...
  const saveCurrentChunk = () => {
    ...
    if (content) {
      ...
      const sectionPath =
        currentTitlePath.length > 0 ? currentTitlePath.join(" > ") + suffix : "Geral" + suffix;
 ...
 */
export function parseExcelToMarkdown(buffer: Buffer, fileName: string): DocumentChunk[] {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    if (dataRows.length === 0) {
      ...
      const mdTable = [`| ${cleanHeaders.join(" | ")} |`, `| ${separatorRow.join(" | ")} |`].join(
        "\n",
      );
    ...
    for (let i = 0; i < dataRows.length; i += ROWS_PER_CHUNK) {
      ...
      const partSuffix = totalParts > 1 ? ` (Parte ${partIndex} de ${totalParts})` : "";
