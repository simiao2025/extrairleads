...
export async function saveOnboardingInfoAction(data: {
	...
	try {
		...
		if (!evolutionUrl) {
			console.error(
				"ERRO CRÍTICO: Configurações da Evolution API URL ausentes no servidor (.env)",
			);
			return {
				success: false,
				error:
					"Serviço de WhatsApp temporariamente indisponível (URL do WhatsApp não configurada).",
			};
		}
