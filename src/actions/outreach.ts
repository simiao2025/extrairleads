			...
			if (provider === "meta_official") {
				...
				const completion = await groq.chat.completions.create({
					messages: [
						{
							role: "system",
							content: config?.agent2Prompt || "Abordagem curta e direta.",
						},
						{
							role: "user",
			...
			try {
				const completion = await groq.chat.completions.create({
					messages: [
						{
							role: "system",
							content: config?.agent2Prompt || "SDR focado em follow-up.",
						},
						{
							role: "user",
							content: `DADOS DO LEAD:
...
export async function generateAiSuggestionAction(leadId: number) {
	...
	const systemPrompt =
		config?.agent2Prompt || "Vendedor B2B direto e amigável.";
